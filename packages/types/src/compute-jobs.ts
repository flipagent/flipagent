 ...
 * `/v1/discover/jobs/*`). Per-pipeline job shapes (`EvaluateJob`,
 * `DiscoverJob`) live next to their request/response counterparts in
 * `evaluate.ts` / `discover.ts`; this file only carries the common
 * status enum + the trace-replay primitives a /stream subscriber sees.
 */
...
export const ComputeJobKind = Type.Union([Type.Literal("evaluate"), Type.Literal("discover")], {
	$id: "ComputeJobKind",
});
...
/**
 * Empty 202-style ack returned by `POST /v1/{evaluate,discover}/jobs` and
 * `POST /v1/{...}/jobs/{id}/cancel`. The full job shape is fetched via
