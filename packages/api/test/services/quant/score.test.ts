...
import { score } from "../../../src/services/quant/score.js";
...
describe("score", () => {
	it("rates buy on a high-margin, high-confidence, liquid listing", () => {
		const s = score(goodListing, market);
	...
	it("rates skip when margin does not clear", () => {
		const tooExpensive = { ...goodListing, priceCents: 11_500 };
		const s = score(tooExpensive, market);
		expect(s.rating).toBe("skip");
		expect(s.reason).toMatch(/below threshold/);
	});

	it("rates watch when margin clears but confidence is low", () => {
		const lowConfidence = { ...goodListing, sellerFeedback: 60, imageCount: 1, descriptionLength: 60 };
		const s = score(lowConfidence, market);
		expect(s.rating).toBe("watch");
	});

	it("rates skip on vetoed listing", () => {
		const newSeller = { ...goodListing, sellerFeedback: 5 };
		const s = score(newSeller, market);
		expect(s.rating).toBe("skip");
	...
	it("rates skip when liquidity floor not cleared", () => {
		...
		const s = score(goodListing, illiquid);
		expect(s.rating).toBe("skip");
	...
	it("emits dollarsPerDay + annualizedRoi when meanDaysToSell present", () => {
		...
		const s = score(goodListing, withDuration);
	...
	it("leaves time-aware fields undefined when meanDaysToSell missing", () => {
		const s = score(goodListing, market); // no meanDaysToSell
	...
	it("annualizedRoi caps at 100 (10000% / yr) for absurdly fast turnover", () => {
		...
		const s = score(goodListing, fast);
	...
	it("uses mean (not median) as the expected sale anchor", () => {
		...
		const a = score(goodListing, skewed);
		const flat: MarketStats = { ...market, meanCents: 12_000 };
		const b = score(goodListing, flat);
