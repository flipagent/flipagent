/**
 * sell/finances ops — payouts (list + summary) + transactions.
 *
 * `transactionType: "transfer"` rows in the transactions stream cover
 * the inter-account transfer events that eBay's standalone
 * `/sell/finances/v1/transfer` endpoint exposes; flipagent doesn't
 * surface a separate `/v1/transfers` route. Power users who need
 * transfer-only fields (`bankReference`, etc.) can hit eBay directly
 * via `client.http.get("/sell/finances/v1/transfer")`.
 */

import type { Payout, PayoutSummary, PayoutsListQuery, Transaction, TransactionsListQuery } from "@flipagent/types";
