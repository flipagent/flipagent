/**
 * Bridge implementation of eBay's Buy Order API. Backs
 * `/v1/buy/order/checkout_session/*` and `/v1/buy/order/purchase_order/*`
 * when `EBAY_ORDER_API_APPROVED=0` — the same public surface eBay's
 * REST exposes, but the actual buy is driven by our Chrome extension
 * via the bridge protocol.
 *
 * eBay's flow is two-stage:
 *
 *   POST /checkout_session/initiate            → CheckoutSession (no exec)
 *   POST /checkout_session/{id}/place_order    → PurchaseOrder (exec)
 *   GET  /purchase_order/{id}                  → poll
 *
 ...
 * buyer's existing eBay defaults; we can't override mid-flight.
 *
 * Status mapping internal → eBay shape:
