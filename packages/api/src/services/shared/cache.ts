/**
 * Postgres-backed response cache. Provider-agnostic — same primitive
 * is used by every transport that wants short-TTL anti-thundering-herd
 * persistence (eBay REST passthrough, scraper, Trading prefetch,
 * future Amazon / Mercari adapters).
 *
 * Hard TTL keeps cached entries fresh enough that they're not
 * "redistribution" but "anti-thundering-herd" — typical 60 minutes
 * for active searches, longer for item detail / category trees.
 *
 * Higher-level callers should use `withCache(args, fetcher)` from
 * `./with-cache.js`; reach for these primitives only when you need
 * raw cache-key control (e.g., custom invalidation jobs).
 */
