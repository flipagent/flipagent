...
import type { DraftRecommendation } from "../evaluate/types.js";
 ...
 *
 * `listPriceRecommendation` is null when the market lacks `meanDaysToSell` — caller
...
export function draft(
	...
	if (!advice) {
		if (!market.meanDaysToSell || market.meanDaysToSell <= 0) {
			reason = `no time-to-sell data among ${market.nObservations} comparables — list at $${(market.medianCents / 100).toFixed(2)} manually`;
		} else if (market.meanCents <= 0) {
			reason = "market mean is zero — no comparables to anchor price";
		} else {
	...
	return { titleSuggestion, listPriceRecommendation: advice, reason };
}
