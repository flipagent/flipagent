...
export async function findEligibleAuctionItems(): Promise<{ items: Array<{ itemId: string }> }> {
	const res = await appRequest<{ items?: Array<{ itemId: string }> }>({
		path: "/buy/offer/v1/find_eligible_items",
	});
