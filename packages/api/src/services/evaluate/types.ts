...
import type { ListPriceRecommendation, MarketStats } from "../quant/index.js";
...
/** p10/p90 of expected net (cents) over the IQR-cleaned comparable cohort. */
export type NetRangeCents = { p10Cents: number; p90Cents: number };

/** Evaluation returned by `evaluate`. All money fields are cents-denominated integers. */
export type Evaluation = {
	/** Expected net take-home in cents (= quant Score.netCents). Mean over the comparable cohort. */
	expectedNetCents: number;
	/** 0..1 — combination of seller trust, image count, comparable count, listing freshness. */
	confidence: number;
	/** Total delivered cost to the destination, or null if no destination state was supplied. */
	landedCostCents: number | null;
	signals: ReadonlyArray<FiredSignal>;
	/** Bottom-line rating: "buy" | "hold" | "skip". */
	 ...
	 * Null when no comparables (no market mean to invert).
	 */
	 ...
	 * the ceiling itself is null (no comparables / market mean = 0).
	 */
	safeBidBreakdown: {
	 ...
	 * sale. Null when fewer than 4 comparables to estimate from.
	 */
	winProbability: number | null;
	/** Risk band on net. Null when fewer than 4 comparables. */
	 ...
	 *
	 * Null when comparables lack duration data (no time-to-sell model) or no
	 * profitable price exists (evaluation will be `skip`).
	 */
	recommendedExit: {
...
/**
 * Bundle returned by `marketSummary(comparables, asks?, context?)`. Distribution
 * stats + EV-optimal list price (when comparables carry duration data).
 */
...
/** Tunables shared by `evaluate`, `discoverDeals`, and `extractSignals`. */
export type EvaluateOptions = {
	 ...
	 * Used in both the bidCeiling math and the per-comparable net distribution.
	 */
