 ...
 * Web Scraper API — a managed service that handles anti-bot / Akamai /
 * JS rendering on Oxylabs's infrastructure. We POST a URL, they return
 * rendered HTML. Reliable for eBay.
 *
 * **No detection-evasion in our code path.** No UA rotation, no
 * fingerprint spoofing, no bot-wall retry loops. If you want to scrape
 * eBay through our API, the Web Scraper API is paying Oxylabs to be
 * the legitimate scraping client; that responsibility (and ToS exposure)
 * sits with the vendor whose business is exactly that. flipagent stays
 * a normal HTTPS client.
 *
...
import { ebayDetailToBrowse } from "../services/listings/transform.js";
