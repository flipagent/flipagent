...
export const webhooksRegisterDescription =
	'Register a webhook endpoint to receive flipagent → caller server-to-server callbacks. Calls POST /v1/webhooks. **When to use** — agent setup phase: subscribe to the events you care about so flipagent can push state changes to your endpoint instead of you polling. **Inputs** — `url` (HTTPS endpoint that receives POST), `events: string[]` from the `WebhookEventType` union: `order.queued`, `order.claimed`, `order.awaiting_user_confirm`, `order.placing`, `order.completed`, `order.failed`, `order.cancelled`, `order.expired` (today the bridge-driven `/v1/purchases` lifecycle is the only emitting surface; the union expands as more sources land). For eBay-marketplace platform notifications (ItemSold, AccountSuspended, etc.) use `flipagent_list_notification_topics` + `flipagent_create_notification_subscription` instead — separate system. **Output** — `{ id, secret }`. **Critical**: store `secret` immediately — flipagent uses it for HMAC signatures and won\'t show it again. The SDK ships a verifier helper. **Prereqs** — `FLIPAGENT_API_KEY`. **Example** — `{ url: "https://my-app.example.com/webhooks/flipagent", events: ["order.completed", "order.failed"] }`.';
