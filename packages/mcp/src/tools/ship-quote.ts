...
export const shipQuoteDescription =
	"Compute total delivered cost via flipagent's forwarder. Calls POST /v1/ship/quote. Pass `item` (any /v1/listings/* listing) and `forwarder` (`destState` US 2-letter, `weightG` grams, optional `dimsCm`, optional `provider`). Returns itemized breakdown plus forwarder ETA + caveats. US-domestic only; tax is 0 — destination-state sales tax is the caller's job.";
