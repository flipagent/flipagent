...
const STEPS: Step[] = [
	{
		...
		code: {
			...
			curl: [
				<>{FN("curl")} {STR('"https://api.flipagent.dev/v1/items/search?q=canon+ef+50mm+1.8&marketplace=ebay_us&limit=25"')} \</>,
				<>{"  "}-H {STR('"X-API-Key: fa_…"')}</>,
			],
			cli: [
	...
	},
	{
		...
		code: {
			python: [
				...
				<>{"  "}json={"{"}</>,
				<>{"    "}{STR('"items"')}: [{"{"}{STR('"itemId"')}: {STR('"v1|3852…|0"')}, {STR('"quantity"')}: {NUM("1")}{"}"}],</>,
				<>{"    "}{STR('"shipTo"')}: {"{"} country: {STR('"US"')}, postalCode: {STR('"97230"')} {"}"},</>,
				<>{"  "}{"}"},</>,
				<>{"  "}headers={"{"}{STR('"X-API-Key"')}: {STR('"fa_…"')}{"}"},</>,
				")",
			],
			node: [
				...
				<>{"  "}items: [{"{"} itemId: {STR('"v1|3852…|0"')}, quantity: {NUM("1")} {"}"}],</>,
				<>{"  "}shipTo: {"{"} country: {STR('"US"')}, postalCode: {STR('"97230"')} {"}"},</>,
				"});",
			],
			curl: [
		...
		result: [
			...
			<>{"  "}{STR('"transport"')}: {STR('"rest"')},</>,
			<>{"  "}{STR('"items"')}: [{"{"} itemId: {STR('"v1|3852…|0"')}, quantity: {NUM("1")} {"}"}],</>,
			<>{"  "}{STR('"pricing"')}: {"{"} total: {"{"} amountCents: {NUM("4700")}, currency: {STR('"USD"')} {"}"} {"}"},</>,
	...
	},
	{
		...
		code: {
			python: [
				<>r = requests.{FN("post")}(</>,
				<>{"  "}{STR('"https://api.flipagent.dev/v1/listings/{sku}"')},</>,
				<>{"  "}json={"{"}</>,
			...
			curl: [
				<>{FN("curl")} -X PUT https://api.flipagent.dev/v1/listings/{"{sku}"} \</>,
				<>{"  "}-H {STR('"X-API-Key: fa_…"')} \</>,
	...
	},
	{
		...
		result: [
			...
			<>{"    "}{STR('"marketplace"')}: {STR('"ebay_us"')},</>,
			<>{"    "}{STR('"status"')}: {STR('"paid"')},</>,
			<>{"    "}{STR('"items"')}: [{"{"} itemId: {STR('"v1|408517|0"')}, quantity: {NUM("1")} {"}"}],</>,
			<>{"    "}{STR('"shipTo"')}: {"{"} country: {STR('"US"')}, state: {STR('"NY"')} {"}"},</>,
			<>{"    "}{STR('"pricing"')}: {"{"} total: {"{"} amountCents: {NUM("9999")}, currency: {STR('"USD"')} {"}"} {"}"},</>,
