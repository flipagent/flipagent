...
const STEPS: Step[] = [
	{
		...
		code: {
			...
			node: [
				...
				<>{"  "}q: {STR('"canon ef 50mm"')}, marketplace: {STR('"ebay"')},</>,
	...
	},
	{
		...
		code: {
			...
			curl: [
				...
				<>{"  "}-H {STR('"X-API-Key: fa_…"')} \</>,
				<>{"  "}-d {STR(`'{"title":"Canon EF 50mm…","price":{"value":9999,"currency":"USD"},"condition":"used_good","categoryId":"15247","images":["…"],"marketplace":"ebay"}'`)}</>,
			],
			cli: [
		...
		plain: {
			...
			curl: 'curl -X POST https://api.flipagent.dev/v1/listings \\\n  -H "X-API-Key: fa_…" \\\n  -d \'{"title":"Canon EF 50mm…","price":{"value":9999,"currency":"USD"},"condition":"used_good","categoryId":"15247","images":["…"],"marketplace":"ebay"}\'',
