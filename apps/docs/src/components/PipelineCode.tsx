...
const STEPS: Step[] = [
	...
	},
	{
		...
		result: [
			...
			<>{"  "}{STR('"evaluation"')}: {"{"}</>,
			<>{"    "}{STR('"rating"')}: {STR('"buy"')},</>,
			<>{"    "}{STR('"reason"')}: {STR('"Net $31.20 at p50, 5 cheaper asks ahead"')},</>,
			...
			<>{"    "}{STR('"recommendedExit"')}: {"{"} {STR('"listPriceCents"')}: {NUM("8500")}, {STR('"expectedDaysToSell"')}: {NUM("5.2")}, {STR('"daysLow"')}: {NUM("3.1")}, {STR('"daysHigh"')}: {NUM("7.3")}, {STR('"queueAhead"')}: {NUM("4")}, {STR('"netCents"')}: {NUM("3120")} {"}"}</>,
			<>{"  "}{"}"},</>,
